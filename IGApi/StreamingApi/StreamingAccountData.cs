    ...
    public sealed partial class ApiEngine
    {
        ...
        private void StreamingAccountDataUpdate(object? sender, UpdateArgs<StreamingAccountData> e)
        {
            try
            {
                ...
                Task.Run(async () => await iGApiDbContext.SaveAccount(streamingAccountData, currentAccountId).SaveChangesAsync()).Wait(); // Use wait, avoiding object disposed DbContext operations are still running.
            }
            catch (Exception ex) 
            {
